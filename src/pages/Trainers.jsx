...
const Trainers = () => {
  ...
  const [applyStep, setApplyStep] = useState('ecosystem'); // 'ecosystem' or 'form'
  const [applyData, setApplyData] = useState({ name: '', age: '', experience: '', location: '', phone: '' });
  
  const [showHireModal, setShowHireModal] = useState(false);
  const [hireModalStep, setHireModalStep] = useState('pitch'); // 'pitch', 'list', 'form'
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
       ...
       <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16 flex flex-col md:flex-row gap-8">
         ...
         <div className="flex-1 relative overflow-hidden rounded-3xl group h-[300px]">
           ...
           <div className="relative h-full p-10 flex flex-col justify-center items-start text-white">
             ...
             <button 
               onClick={() => { setApplyStep('ecosystem'); setShowApplyModal(true); }} 
         ...
         <div className="flex-1 relative overflow-hidden rounded-3xl group h-[300px]">
           ...
           <div className="relative h-full p-10 flex flex-col justify-center items-start text-white">
             ...
             <button 
               onClick={() => { setHireModalStep('pitch'); setShowHireModal(true); }} 
               className="bg-white text-black font-black py-4 px-8 rounded-xl uppercase tracking-widest hover:bg-yellow-500 transition-all shadow-xl active:scale-95"
             >
               Hire Now
             </button>
      ...
      {/* Apply Modal (Premium Trainer Ecosystem + Registration Form) */}
      <AnimatePresence>
        {showApplyModal && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            onClick={() => { setShowApplyModal(false); setApplyStep('ecosystem'); }}
          >
            <motion.div
                  ...
                  {applyStep === 'ecosystem' ? (
                    ...
                    >
                      <div className="text-center">
                        <button 
                          onClick={() => setApplyStep('ecosystem')}
                        ...
                        <h2 className="text-4xl font-black text-gray-900 mb-2 uppercase tracking-tighter leading-none">Trainer <span className="text-emerald-600">Application</span></h2>
                        <p className="text-gray-500 font-medium italic">Join the Colfit.in elite network.</p>
                      </div>
                      
                      <form
                        onSubmit={e => {
                          e.preventDefault();
                          const msg = `🌟 New Trainer Registration:\n\n👤 Full Name: ${applyData.name}\n🎂 Age: ${applyData.age}\n🏆 Experience: ${applyData.experience}\n📍 Location: ${applyData.location}\n📞 Contact: ${applyData.phone}`;
                      ...
                      >
                          ...
                          {[
                            ...
                            <div key={field.id}>
                              ...
                              <input
                                ...
                                type={field.type}
                                value={applyData[field.id]}
                                onChange={e => setApplyData({ ...applyData, [field.id]: e.target.value })}
                        ...
                        <div className="pt-4">
                          <button 
                            type="submit" 
                            className="w-full bg-emerald-600 text-white py-6 rounded-2xl font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-[0_20px_40px_-10px_rgba(16,185,129,0.4)] flex items-center justify-center gap-3 active:scale-[0.98]"
                          >
                            Submit Registration
                            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>
                          </button>
      ...
      {/* Hire Modal (Premium Supply & Grow Pitch + Trainer Selection + Form) */}
      <AnimatePresence>
        {showHireModal && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[101] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md"
            onClick={() => { setShowHireModal(false); setHireModalStep('pitch'); setHireSelectedTrainer(null); }}
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0, y: 30 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
                ...
                <AnimatePresence mode="wait">
                          ...
                          <div key={i} className="group bg-gray-50 p-6 rounded-[1.5rem] border border-gray-100 hover:bg-white hover:shadow-xl hover:border-transparent transition-all duration-300">
                            ...
                            <ul className="space-y-2.5 mb-6">
                              {card.items.map((item, i) => (
                                <li key={i} className="flex items-center gap-2 text-[11px] text-gray-600 font-bold">
                  ...
                  {hireModalStep === 'list' && (
                    ...
                    >
                      <div className="text-center max-w-xl mx-auto">
                        <button 
                          onClick={() => setHireModalStep('pitch')}
                        ...
                        <h2 className="text-4xl font-black text-gray-900 mb-2 uppercase tracking-tighter leading-none">Available <span className="text-indigo-600">Talent</span></h2>
                        <p className="text-gray-500 font-medium italic">Select an elite professional to elevate your facility.</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pb-10">
                        {trainers.map(tr => (
                          <div
                            key={tr.id}
                            className="group p-8 bg-gray-50 border-2 border-transparent rounded-[2rem] hover:bg-white hover:border-indigo-600 hover:shadow-2xl transition-all cursor-pointer relative overflow-hidden"
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <span className="bg-indigo-50 text-indigo-700 text-[9px] font-black px-2.5 py-1 rounded-lg uppercase tracking-tighter">Experience: {tr.experience}</span>
                  ...
                  {hireModalStep === 'form' && (
                    ...
                    >
                      <div className="text-center">
                        ...
                        <h2 className="text-4xl font-black text-gray-900 mb-2 uppercase tracking-tighter leading-none">Hire <span className="text-indigo-600">{hireSelectedTrainer?.name}</span></h2>
                      ...
                      >
                            ...
                            <div key={field.id} className={field.id === 'requirement' ? 'md:col-span-2' : ''}>
                              ...
                              {field.type === 'textarea' ? (
                                <textarea
                                  ...
                                  className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl px-6 py-5 text-gray-900 focus:outline-none focus:border-indigo-600 focus:bg-white transition-all font-bold placeholder:text-gray-300 min-h-[100px] resize-none"
                                  required
                                />
                              ) : (
                                <input
                        ...
                        <div className="pt-4">
                          <button 
                            type="submit" 
                            className="w-full bg-indigo-600 text-white py-6 rounded-2xl font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-[0_20px_40px_-10px_rgba(79,70,229,0.4)] flex items-center justify-center gap-3 active:scale-[0.98]"
                          >
